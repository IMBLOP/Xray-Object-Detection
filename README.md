...
본 프로젝트는 초기 베이스라인 모델(V0)의 한계를 분석하고, 데이터 분포 조정 및 전처리 고도화를 통해 성능을 단계적으로 개선하였습니다.

### 4.1. 버전별 실험 요약

| 구분 | V0 (Baseline) | V1 (Distribution Fix) | V2 (Optimization) |
| :--- | :--- | :--- | :--- |
| **데이터 구성** | 기존 Split 사용 | **전체 병합 후 8:1:1 재분할** | 8:1:1 재분할 유지 |
| **데이터 증강** | 기본 설정 | **Mixup(0.1), CopyPaste(0.3), Rotation(10.0)** | V1 증강 설정 유지 |
| **좌표 정규화** | XML 헤더 기준 | XML 헤더 기준 | **실제 이미지 크기 기준 (정밀화)** |
| **타겟 클래스** | 16종 (소형 물체 포함) | 16종 (소형 물체 포함) | **15종 (탐지 용이성 고려 재구성)** |
| **결과 (mAP50)** | Test 35.5% (과적합) | **Test 99.2% (성공)** | 진행 중 |

### 4.2. 상세 진행 과정

#### V0: 초기 베이스라인
* **내용:** AI HUB에서 제공하는 기본 데이터셋 구조를 그대로 전처리하여 학습.
* **문제점:** 학습 mAP는 99%에 달했으나, 평가 mAP는 **35.5%로** 매우 저조.
* **원인 분석:** Train 데이터와 Test 데이터 간의 **심각한 데이터 분포 불일치(Domain Shift)** 확인.

#### V1: 데이터 분포 재구성 및 증강 
* **개선 사항:**
    * **데이터 재분할:** 문제라고 판단한 Test 데이터를 제외하고 Train/Validation 데이터를 모두 합친 후, 전체를 무작위로 섞어 **Train(8) : Val(1) : Test(1)** 비율로 재분할하여 데이터 분포 불균형 해결.
    * **증강 기법 도입:** X-ray 특성(물체 겹침, 회전)을 고려하여 `CopyPaste`, `Mixup`, `Degrees` 옵션 추가.
* **결과:** **Test mAP50 99.2%, mAP50-95 92.8%** 달성. 과적합 문제를 해결하고 일반화 성능 확보 성공.

#### V2: 전처리 정밀화 및 타겟 최적화
* **목표:** 탐지 정확도의 정밀성 향상 및 현실적인 탐지 타겟 재설정.
* **주요 변경 사항:**
    1.  **전처리 로직 수정:** 리사이징 과정에서 XML 헤더 정보와 실제 이미지 크기 간의 미세 오차를 방지하기 위해, **실제 변환된 이미지 크기를 읽어 좌표를 정규화**하도록 로직 개선.
    2.  **클래스 재정의 (15종):** 탐지가 어려운 초소형 물체 등을 제외하고 실효성 높은 품목으로 클래스 재구성.
        * *Target Classes:* `Aerosol`, `Axe`, `Bat`, `Battery`, `Gun`, `Hammer`, `HDD`, `Knife`, `LapTop`, `MetalPipe`, `Scissors`, `SmartPhone`, `Spanner`, `TabletPC`, `USB`
* **모델:** 현재 `YOLOv8m` 및 `YOLOv8l` (Large) 모델로 학습 진행 중.

---

## 5. 실험 결과 및 모델 성능 

본 프로젝트는 V0, V1, V2 단계에 걸쳐 모델의 성능을 지속적으로 개선하였습니다. 
